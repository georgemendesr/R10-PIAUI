...
class AdsService {
  ...
  getBanners(): Banner[] {
    ...
    if (!stored) {
      console.log('Nenhum banner no localStorage, criando banners padrão...'); // Debug
      banners = this.getDefaultBanners();
      this.saveBanners(banners); // Salvar os banners padrão
    } else {
      banners = JSON.parse(stored);
    }
    
    console.log('Banners carregados do localStorage:', banners); // Debug
    return banners;
  }
  ...
  getBannersByPosition(posicao: Banner['posicao']): Banner[] {
    ...
    console.log(`Buscando banners para posição: ${posicao}`); // Debug
    console.log('Data atual:', now); // Debug
    
    const filteredBanners = banners
      .filter(banner => {
        console.log(`Verificando banner ${banner.id}:`, {
        ...
        if (banner.posicao !== posicao) {
          console.log(`Banner ${banner.id} rejeitado: posição incorreta`);
          return false;
        }
        
        // Filtrar por status
        if (banner.status === 'pausado') {
          console.log(`Banner ${banner.id} rejeitado: status pausado`);
          return false;
        }
        ...
        if (now < inicio || now > fim) {
          console.log(`Banner ${banner.id} rejeitado: fora do período`);
          return false;
        }
        
        // Verificar limites de impressões/cliques
        if (banner.impressoesMax && banner.impressoesAtuais >= banner.impressoesMax) {
          console.log(`Banner ${banner.id} rejeitado: limite de impressões atingido`);
          return false;
        }
        if (banner.cliquesMax && banner.cliquesAtuais >= banner.cliquesMax) {
          console.log(`Banner ${banner.id} rejeitado: limite de cliques atingido`);
          return false;
        }
        
        console.log(`Banner ${banner.id} aprovado!`);
        return true;
      })
      .sort((a, b) => a.prioridade - b.prioridade); // Ordenar por prioridade
      
    console.log(`Banners filtrados para ${posicao}:`, filteredBanners); // Debug
  ...
  getActiveBanner(posicao: Banner['posicao']): Banner | null {
    ...
    if (banners.length === 0) {
      console.log(`Nenhum banner ativo encontrado para posição: ${posicao}`); // Debug
      return null;
    }
    ...
    console.log(`Banner selecionado para ${posicao}:`, selectedBanner); // Debug
  ...
  resetBanners(): void {
    ...
    console.log('🔄 Banners resetados - próxima chamada criará banners padrão');
  }
